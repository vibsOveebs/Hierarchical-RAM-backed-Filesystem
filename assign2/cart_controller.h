...
// controller register
typedef uint16_t CartridgeIndex; // Index number of cartridge 0-64
typedef uint16_t CartFrameIndex; // Frame index in current cartridge 0 - 1024
typedef char CartFrame[CART_FRAME_SIZE]; // A Frame  1024
typedef CartFrame CartCartridge[CART_CARTRIDGE_SIZE]; // A Cartridge 1024x1024



/*

 uint64_t Specification (See slides)
...
uint64_t cart_io_bus(uint64_t regstate, void *buf);
